...
const test = {  
    ...
    {name:"state", type:"select", options : stateData,value:"", parent:"country"},
...
const test2 = {
    ...
    country:{ label:"Country",type:"select", options:countryData, prop:"",pos:2},
    state:{label:"State/region", type:"select", options : stateData, parent:"country", prop:"",pos:3},
    Number:{ label:"Number", type:"select", options : [{display:"1",prop:1},{display:"2",prop:2},{display:"3",prop:3}], prop:"",pos:4} },
    key:"123435"
...
export default class AACNform extends Component {
    ...
    handleChange(event){
        ...
        this.setState(prevState=>({fields:{...prevState.fields,[event.target.name]:{...prevState.fields[event.target.name],prop:[event.target.value]} }}))
    }
...
const fieldset = []
    for(let key in this.state.fields){
        ...
        fieldset.push( fields[key].type=="select"?(<label pos={fields[key].pos}>{fields[key].label}
                   
                   <select onChange={this.handleChange} name={key} style={{width:"200px",color:"black"}} >
                   {fields[key].options.
                   map(el=><option value={el}>{el.display}</option>).
                   filter(e=>fields[key].options.parent?fields[key].options.parent.value.key===e.pk:e )}
                   </select>
                   </label>):(
                   <label  pos={fields[key].pos}>{fields[key].label}
                   <input onChange={this.handleChange} name={key} type={fields[key]} value={fields[key].prop}/>
                   </label>)
    )
                   } 
